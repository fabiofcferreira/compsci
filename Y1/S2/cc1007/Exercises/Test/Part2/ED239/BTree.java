public class BTree<T> {   
  ...
  // Number of single children nodes (nodes with no brothers nodes)
  public int count() {
    return count(root);
  }
  ...
  // Lower depth level where it is possible to find the value v
  public int level(T v) {
    return level(root, 0, v);
  }

  private int level(BTNode<T> node, int depth, T v) {
    ...
    int left = level(node.getLeft(), depth + 1, v);
    int right = level(node.getRight(), depth + 1, v);
